...
class ArticleBiasIndicatorBlock(AbstractBaseBlock):
    ...
    def transform(
        ...
        bias_cdf = (
            trans_cdf.groupby("article_id")["customer_id"].agg(["nunique", "count"]).reset_index()
        )
        bias_cdf.columns = ["article_id", "nunique_customers", "sales"]

        bias_cdf["sales_by_buy_users"] = (
            bias_cdf["sales"] / bias_cdf["nunique_customers"]
        )  # その商品購入する人たちの中ではどれくらい買われるか？
...
class FirstBuyDateBlock(AbstractBaseBlock):
    ...
    def transform(
        ...
        date_item_first_purchased["first_purchased_day"] = pd.to_datetime(
            date_item_first_purchased["first_purchased_day"].to_array()
        )

        date_item_first_purchased["days_from_first_purchased"] = (
            end_date_datetime - date_item_first_purchased["first_purchased_day"]
        ).dt.days
...
class SalesPerTimesForArticleBlock(AbstractBaseBlock):
    ...
    def preprocess(self, trans_cdf: cudf.DataFrame):
        ...
        trans_cdf["dayofweek"] = trans_cdf[
            "t_dat_datetime"
        ].dt.dayofweek  # 月曜日が0, 日曜日が6 ref: https://pandas.pydata.org/docs/reference/api/pandas.Series.dt.dayofweek.html
        return trans_cdf
    ...
    def make_agg_sales(self, trans_cdf, groupby_name, groupby_cols):
        ...
        sales_cdf = (
            sales_by_groupby_cols.groupby("article_id")["sales"].agg(self.agg_list).reset_index()
        )
...
class SalesPerDayCustomerBlock(AbstractBaseBlock):
    ...
    def transform(
        ...
        out_cdf = (
            trans_sales_cdf.groupby("customer_id")["all_sales_per_day"]
            .agg(self.agg_list)
            .reset_index()
        )
        out_cdf.columns = ["customer_id"] + [
            f"sales_in_day_{col}" for col in out_cdf.columns[1:]
        ]  # 1列目はcustomer_id
    ...
    def sales_day(self, trans_cdf):
        ...
        sales_cust_day = trans_cdf.drop_duplicates(subset=["t_dat", "customer_id"])[
            ["t_dat", "customer_id"]
        ]
...
class RepeatCustomerBlock(AbstractBaseBlock):
    ...
    def transform(
        ...
        # TODO : 他にも活かし方あるかも？
        repeat_customer_df = (
            t_dat_df.groupby("customer_id")["repeat_flg"].agg(["sum", "mean"]).reset_index()
        )
    ...
    def preprocess(self, trans_cdf):
        ...
        t_dat_cdf = (
            trans_cdf.sort_values(["customer_id", "article_id", "t_dat_datetime"])
            .drop_duplicates(subset=["customer_id", "article_id", "t_dat_datetime"], keep="first")[
                ["customer_id", "article_id", "t_dat_datetime"]
            ]
            .reset_index()
        )
        t_dat_df = t_dat_cdf.to_pandas()
        t_dat_df["shift_t_dat_datetime"] = t_dat_df.groupby(["customer_id", "article_id"])[
            "t_dat_datetime"
        ].shift(1)
        t_dat_df["day_diff"] = (
            t_dat_df["t_dat_datetime"] - t_dat_df["shift_t_dat_datetime"]
        ).dt.days
...
class MostFreqBuyDayofWeekBlock(AbstractBaseBlock):
    ...
    def transform(
        ...
        out_cdf = sales_by_dayofweek.sort_values(
            ["customer_id", "sales"], ascending=False
        ).drop_duplicates(subset=["customer_id"], keep="first")[["customer_id", "dayofweek"]]
        return out_cdf
...
class CustomerBuyIntervalBlock(AbstractBaseBlock):
    ...
    def make_day_diff(self, trans_cdf):
        t_dat_cdf = (
            trans_cdf.sort_values(["customer_id", "t_dat_datetime"])
            .drop_duplicates(subset=["customer_id", "t_dat_datetime"], keep="first")[
                ["customer_id", "t_dat_datetime"]
            ]
            .reset_index()
        )
        t_dat_df = t_dat_cdf.to_pandas()
        t_dat_df["shift_t_dat_datetime"] = t_dat_df.groupby("customer_id")["t_dat_datetime"].shift(
            1
        )
        t_dat_df["day_diff"] = (
            t_dat_df["t_dat_datetime"] - t_dat_df["shift_t_dat_datetime"]
    ...
    def continue_buy(self, t_dat_df):
        ...
        t_dat_df = t_dat_df.sort_values(
            ["customer_id", "continue_buy_flg"], ascending=False
        ).drop_duplicates(subset=["customer_id"], keep="first")[
            ["customer_id", "continue_buy_flg"]
        ]
        return t_dat_df

    def interval_describe(self, t_dat_df):
        day_diff_describe = (
            t_dat_df.groupby("customer_id")["day_diff"].agg(self.agg_list).reset_index()
        )
...
class PostalCodeBlock(AbstractBaseBlock):
    ...
    def transform(
        ...
        trans_postal_cdf = trans_cdf.merge(
            cust_cdf[["customer_id", "postal_code"]], on="customer_id", how="left"
        )

        # agg_colsの計算(cust info)
        out_agg_cust_cdf = (
            cust_cdf.groupby("postal_code")[self.agg_cust_cols].agg(self.agg_list).reset_index()
        )
        out_agg_cust_cdf.columns = ["postal_code"] + [
            f"postal_code_{i[0]}_{i[1]}" for i in list(out_agg_cust_cdf.columns[1:])
        ]  # 1列はpostal_code

        # agg_colsの計算(trans info)
        out_agg_trans_cdf = (
            trans_postal_cdf.groupby("postal_code")[self.agg_trans_cols]
            .agg(self.agg_list)
            .reset_index()
        )
        out_agg_trans_cdf.columns = ["postal_code"] + [
            f"postal_code_{i[0]}_{i[1]}" for i in list(out_agg_trans_cdf.columns[1:])
        ]  # 1列はpostal_code

        # 性別特徴量
        out_sex_cdf = self.sex_per_postal_code(
        ...
        out_cdf = (
            out_agg_cust_cdf.merge(out_agg_trans_cdf, on="postal_code", how="left")
    ...
    def sex_per_postal_code(self, trans_cdf, art_cdf, cust_cdf, y_cdf, target_customers, logger):
        ...
        if "men_percentage" in cust_cdf:
            cust_sex_cdf = cust_cdf[
                ["customer_id", "postal_code", "women_percentage", "men_percentage"]
            ].copy()
        else:
            # 特徴量を作成する
            cust_sex = SexCustomerBlock("customer_id")
            cust_sex_cdf_no_postal = cust_sex.transform(
                trans_cdf, art_cdf, cust_cdf, y_cdf, target_customers, logger
            )

            # postal_code情報を追加する
            cust_sex_cdf = cust_cdf[["customer_id", "postal_code"]].merge(
                cust_sex_cdf_no_postal, on="customer_id", how="left"
            )
        # 計算
        out_sex_cdf = (
            cust_sex_cdf.groupby("postal_code")[["women_percentage", "men_percentage"]]
            .agg(self.agg_list)
            .reset_index()
        )
        out_sex_cdf.columns = ["postal_code"] + [
            f"postal_code_{i[0]}_{i[1]}" for i in list(out_sex_cdf.columns[1:])
        ]  # 1列はpostal_code
        return out_sex_cdf

    def most_popular_item(self, trans_postal_cdf):
        popular_article_cdf = (
            trans_postal_cdf.groupby(["postal_code", "article_id"]).size().reset_index()
        )
        popular_article_cdf.columns = ["postal_code", "article_id", "sales_in_postal_code"]
        # 一番人気の商品を入れる
        # TODO: もう少し工夫しても良いかも, 同率1位は考えられていない
        most_popular_article_cdf = popular_article_cdf.sort_values(
            "sales_in_postal_code", ascending=False
        ).drop_duplicates(subset=["postal_code"])[["postal_code", "article_id"]]
