...
def read_cdf(input_dir, DRY_RUN):
    if not DRY_RUN:
        ...
        cust_cdf = cudf.read_parquet(input_dir / f"customers_sample_{sample}.parquet")
        art_cdf = cudf.read_parquet(
...
def phase_date(diff_days: int):
    ...
    return datetime.combine(last_date - timedelta(days=diff_days), datetime.min.time())
...
def squeeze_pred_df_to_submit_format(
    pred_df: pd.DataFrame, fill_logic=None, args=None
) -> pd.DataFrame:
    """
    ...
    submit_format_df = (
        pred_df.sort_values(by=["customer_id", "pred"], ascending=False)
    ...
    if "target" in pred_df.columns:
        submit_format_df["target"] = (
            pred_df[pred_df["target"] == 1]
            .groupby("customer_id")["article_id"]
            .apply(list)
            .apply(fill_logic, args=args)
            .apply(arts_id_list2str)
...
def convert_sub_df_customer_id_to_str(
    input_dir: Path, sub_fmt_df: pd.DataFrame
) -> pd.DataFrame:
    ...
    hex_id2idx_dic = {
        v: k
        for k, v in customer_hex_id_to_int(sample_df["customer_id"]).to_dict().items()
    }
