...
def train_rank_cat(train_df, valid_df, param):
    ...
    # catboostはcategory → int, floatに置換する必要がある
    train_X[train_X.select_dtypes("category").columns] = train_X[
        train_X.select_dtypes("category").columns
    ].astype("str")
    valid_X[valid_X.select_dtypes("category").columns] = valid_X[
        valid_X.select_dtypes("category").columns
    ].astype("str")

    # catboost Rankerは queryの長さを示すpd.Seriesを渡す必要がある
    train_groups = train_df["customer_id"].astype(str) + "_" + train_df["target_week"].astype(str)
    valid_groups = valid_df["customer_id"].astype(str) + "_" + valid_df["target_week"].astype(str)

    clf = CatBoost(params=param)
    clf_train = Pool(data=train_X, label=train_y, group_id=train_groups, cat_features=cat_features)
    clf_valid = Pool(data=valid_X, label=valid_y, group_id=valid_groups, cat_features=cat_features)
    clf.fit(clf_train, eval_set=[clf_valid])

    val_pred = clf.predict(clf_valid)
    fe = clf.get_feature_importance(clf_train)
    return clf, val_pred, fe


clf, val_pred, fe = train_rank_cat(
    train_df,
    valid_df,
    CAT_BOOST_RANK_PARAM,
)
...
if not isinstance(clf,catboost.core.CatBoost):
    #catboost未対応 :pray
    fig, ax = visualize_importance([clf], train_X)
    fig.savefig(log_dir / "feature_importance.png")
    plt.show()

if isinstance(clf,catboost.core.CatBoost):
    cat_path = log_dir / f"cat_{mapk_val}.txt"
...
for bucket in tqdm(range(0, len(sub_customer_ids), BATCH_USER_SIZE)):
    ...
    if isinstance(clf, catboost.core.CatBoost):
        # catboostはcategory列をstrで利用する
        batch_test_X = batch_test_X.astype(str)
