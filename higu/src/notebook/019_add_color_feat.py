...
def make_train_valid_df(
    ...
    for target_week in tqdm(target_weeks):
        ...
        logger.info(f"y_cdfのユニークユーザー数: {y_cdf['customer_id'].nunique()}")
        logger.info(f"base_dfのユニークユーザー数: {base_df['customer_id'].nunique()}")
        logger.info(f"ユーザー数カバレッジ率: {round(100* (base_df['customer_id'].nunique() / y_cdf['customer_id'].nunique()),2)}")
