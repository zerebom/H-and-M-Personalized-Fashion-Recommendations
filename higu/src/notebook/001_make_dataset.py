...
# %%
# データセットの読み込み
trans_df = cudf.read_csv(input_dir / "transactions_train.csv").to_pandas()
...
trans_df["week"] = 104 - (trans_df["t_dat"].max() - trans_df["t_dat"]).dt.days // 7
...
cust_df["club_member_status"] = Categorize().fit_transform(
    cust_df[["club_member_status"]]
)["club_member_status"]
cust_df["postal_code"] = Categorize().fit_transform(cust_df[["postal_code"]])[
    "postal_code"
]
cust_df["fashion_news_frequency"] = Categorize().fit_transform(
    cust_df[["fashion_news_frequency"]]
...
trans_df_sample = trans_df[trans_df["customer_id"].isin(cust_df_sample_ids)]
art_df_sample_ids = set(trans_df_sample["article_id"])
art_df_sample = art_df[art_df["article_id"].isin(art_df_sample_ids)]


cust_df_sample.to_parquet(input_dir / f"customers_sample_{sample}.parquet", index=False)
trans_df_sample.to_parquet(
    input_dir / f"transactions_train_sample_{sample}.parquet", index=False
)
art_df_sample.to_parquet(
    input_dir / f"articles_train_sample_{sample}.parquet", index=False
)
...
pd.to_pickle(
    dict(val_week_purchases_by_cust), output_dir / "val_week_purchases_by_cust.pkl"
)
...
valid_gt = (
    customer_hex_id_to_int(sample_sub.customer_id)
