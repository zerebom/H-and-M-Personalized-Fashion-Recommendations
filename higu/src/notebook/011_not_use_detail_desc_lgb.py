...
def make_y_cdf(
        ...
        start_date: datetime,
        end_date: datetime) -> cudf.DataFrame:
    y_cdf = transactions.query(
        f'@start_date<t_dat<@end_date')[['customer_id', 'article_id']].drop_duplicates()
    y_cdf['purchased'] = 1
    return y_cdf

def clip_transactions(transactions: cudf.DataFrame, end_date: datetime)-> cudf.DataFrame:
...
#%%
# 精度の確認
mapk_val = mapk(val_sub_fmt_df['prediction'].map(lambda x: x.split()),
